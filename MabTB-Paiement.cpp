...
{
	return "SELECT * FROM [dbo].[Paiement]";
}
...
{
	return "DELETE FROM [dbo].[Paiement] WHERE id_paiement = ('" + this->id_paiement + "')";
}
System::String^ NS_Comp_Mappage10::CLmapTB::Update(void)
{
	return "UPDATE [dbo].[Paiement] SET montant = '" + this->montant + "', moyen_paiement = '" + this->moyen_paiement + "', date_paiement = '" + this->date_paiement + "', date_solde_reglement = '" + this->date_solde_reglement + "', id_commande = '" + this->id_commande + "')";
}
